...
import 'package:personal/features/expenses/expense_analytics.dart';
import 'package:personal/features/location/work_arrival_stats.dart';

/// Sanity checks for computed dashboard metrics, logged to [AppLog].
  ...
  static List<String> validateWorkAttendance(WorkArrivalStats workStats) {
    ...
    for (final arrival in workStats.lateArrivals) {
      ...
      if (arrival.scheduledArrival != null &&
          arrival.effectiveOfficeEntry != null &&
          !arrival.effectiveOfficeEntry!.isAfter(arrival.scheduledArrival!)) {
        warnings.add(
          'On-time or early office entry on ${arrival.date} appears in late arrivals.',
        );
      } else if (arrival.scheduledArrival != null &&
          arrival.effectiveOfficeEntry == null &&
