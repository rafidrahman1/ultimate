...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final effectiveChecklistId = resolveSelectedChecklistResultId(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          _HomeChecklistAppBarButton(
            results: withChecklist,
            selectedId: effectiveChecklistId,
          ),
        ],
      ),
...
class _HomeChecklistAppBarButton extends ConsumerStatefulWidget {
  const _HomeChecklistAppBarButton({
  ...
  @override
  ConsumerState<_HomeChecklistAppBarButton> createState() =>
      _HomeChecklistAppBarButtonState();
}

class _HomeChecklistAppBarButtonState
    extends ConsumerState<_HomeChecklistAppBarButton> {
  ...
  Future<void> _showResultPicker() async {
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                SizedBox(
                  width: 24,
                  child: result.id == widget.selectedId
                      ? HomeChecklistIcon(
                          selected: true,
                          size: 20,
                        )
    ...
    if (!mounted) return;
    final title =
        widget.results.firstWhere((r) => r.id == picked).title;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Home checklist: $title')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      ...
      tooltip: hasResults
          ? 'Open checklist · long press to choose report'
          : 'Weekly checklists',
      onPressed: () =>
